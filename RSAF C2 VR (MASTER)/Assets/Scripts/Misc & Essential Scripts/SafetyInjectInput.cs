...
public class SafetyInjectInput : MonoBehaviour
{
    ...
    public void SafeInjectStart()
    {
        if (activated == false)
        {
            ...
            {
                if (safetyInjectors[safetynum].useInject)
                {
                    //from here
                    ...
                    {
                        do { RNG = Random.Range(0, (count - 1)); } while (safetyInjectors[safetynum].targetnumber.Contains(RNG));
